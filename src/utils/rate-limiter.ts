...
export class RateLimiter {
  ...
  async acquire(estimatedTokens: number = 1000): Promise<void> {
    ...
    // 2. Wait for rate limit availability (requests/min and tokens/min).
    await this.waitForRateLimit(estimatedTokens);
    // 3 & 4. Reserve the slot and record this request for the sliding window.
    this.activeRequests++;
  ...
  private async waitForSlot(): Promise<void> {
    // If a slot is already free, don't queue at all — otherwise every
    // caller would wait for a release() that may never come (e.g. on the
    // very first call, nothing has been released yet).
  ...
  private async waitForRateLimit(estimatedTokens: number): Promise<void> {
    while (!this.canProceed(estimatedTokens)) {
      ...
      if (this.requestHistory.length === 0) {
        // Nothing in the window (e.g. concurrency is the only blocker,
        // which waitForSlot already handles) — nothing to wait out.
        break;
      }

      const oldestTimestamp = this.requestHistory[0].timestamp;
      const expiresAt = oldestTimestamp + 60000;
      const rawWaitTime = expiresAt - Date.now() + 100; // small buffer
      const waitTime = Math.min(Math.max(100, rawWaitTime), 5000);

      await sleep(waitTime);
      // Loop re-checks canProceed() after sleeping.
    }
  }
